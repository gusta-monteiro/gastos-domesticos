...
function loadMonth(m, y) {
  ...
  return {
    renda: "",
    cats: CATS.map((c) => ({ key: c.key, pct: c.pct, items: [] })),
  };
}
function saveMonth(m, y, d) {
  localStorage.setItem("fin_" + mKey(m, y), JSON.stringify(d));
}
function getApiKey() {
  return localStorage.getItem("fin_api_key") || "";
}
function setApiKey(k) {
  localStorage.setItem("fin_api_key", k);
}
...
function renderMetrics(md, renda) {
  const totalLancado = md.cats.reduce(
    (s, c) =>
      s + c.items.reduce((ss, it) => ss + (parseFloat(it.value) || 0), 0),
    0,
  );
  const saldo = renda - totalLancado;
  const totalAlocado = md.cats.reduce(
    (s, c) => s + (renda * (parseFloat(c.pct) || 0)) / 100,
    0,
  );
...
function renderCategories(md, renda) {
  ...
  document.getElementById("pct-total-lbl").textContent =
    `${totalPct.toFixed(0)}% alocados`;
  ...
  md.cats.forEach((cat, ci) => {
    ...
    const lancado = cat.items.reduce(
      (s, it) => s + (parseFloat(it.value) || 0),
      0,
    );
    ...
    div.querySelectorAll(".item-name").forEach((inp) => {
      inp.addEventListener("input", (e) => {
        md.cats[parseInt(e.target.dataset.ci)].items[
          parseInt(e.target.dataset.ii)
        ].name = e.target.value;
        saveMonth(curMonth, curYear, md);
      });
    });
    ...
    div.querySelectorAll(".del-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        md.cats[parseInt(btn.dataset.ci)].items.splice(
          parseInt(btn.dataset.ii),
          1,
        );
        saveMonth(curMonth, curYear, md);
        renderCalc();
...
function renderPie(md, renda) {
    ...
    pieChart = new Chart(document.getElementById("pieChart"), {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => `${fmt(ctx.raw)} (${pct(ctx.raw, renda)}%)`,
            },
          },
        },
      },
  ...
  document.getElementById("legend").innerHTML = md.cats
    .map((cat, ci) => {
      const def = CATS[ci];
...
function renderPeriod() {
  ...
  const labels = months.map(
    ({ m, y }) => MONTHS[m].slice(0, 3) + "/" + String(y).slice(2),
  );
  const rendas = months.map(
    ({ m, y }) => parseFloat(loadMonth(m, y).renda) || 0,
  );
  const gastos = months.map(({ m, y }) => {
    const d = loadMonth(m, y);
    return d.cats.reduce(
      (s, c) =>
        s + c.items.reduce((ss, it) => ss + (parseFloat(it.value) || 0), 0),
      0,
    );
  ...
  barChart = new Chart(document.getElementById("barChart"), {
    ...
    options: {
      ...
      scales: {
        x: {
          ticks: { font: { family: "Inter", size: 11 }, autoSkip: false },
          grid: { display: false },
        },
        y: {
          ticks: {
  ...
  const catDatasets = CATS.map((def, ci) => ({
    ...
    data: months.map(({ m, y }) => {
      const d = loadMonth(m, y);
      return d.cats[ci]
        ? d.cats[ci].items.reduce((s, it) => s + (parseFloat(it.value) || 0), 0)
        : 0;
    }),
...
document
  ...
  .addEventListener("change", renderPeriod);
...
document
  ...
  .addEventListener("change", renderReport);
...
function gerarParecer(months) {
  ...
  if (data.length === 0) {
    document.getElementById("ai-output").textContent =
      "Nenhum dado encontrado no período selecionado. Preencha a renda e os gastos nos meses para visualizar o parecer.";
    return;
  }
  ...
  if (taxaMedia <= 75) {
    linhas.push(
      `✔ Taxa média de gasto em ${taxaMedia.toFixed(1)}% da renda — dentro de um patamar saudável.`,
    );
  } else if (taxaMedia <= 90) {
    linhas.push(
      `⚠ Taxa média de gasto em ${taxaMedia.toFixed(1)}% da renda — atenção, margem de segurança reduzida.`,
    );
  } else {
    linhas.push(
      `✘ Taxa média de gasto em ${taxaMedia.toFixed(1)}% da renda — comprometimento elevado, revisar categorias.`,
    );
  }

  if (mesesNegativo > 0) {
    linhas.push(
      `${mesesNegativo} de ${n} ${n === 1 ? "mês" : "meses"} analisados fechou no negativo.`,
    );
  } else {
    linhas.push(
      `Todos os ${n} ${n === 1 ? "mês analisado fechou" : "meses analisados fecharam"} com saldo positivo.`,
    );
  }
  ...
  const catAgg = CATS.map((def, ci) => {
    const totalLancado = data.reduce(
      (s, d) => s + (d.catVals[ci]?.lancado || 0),
      0,
    );
    const totalAlocado = data.reduce(
      (s, d) => s + (d.catVals[ci]?.alocado || 0),
      0,
    );
  ...
  catAgg.forEach((c) => {
    ...
    const status =
      c.uso > 100
        ? `acima do limite (${c.uso.toFixed(0)}% do alocado)`
        : c.uso > 85
          ? `próximo do limite (${c.uso.toFixed(0)}% do alocado)`
  ...
  const maisEstourada = catAgg
    .filter((c) => c.totalAlocado > 0)
    .sort((a, b) => b.uso - a.uso)[0];
  const menorUso = catAgg
    .filter((c) => c.totalAlocado > 0 && c.uso < 50)
    .sort((a, b) => a.uso - b.uso)[0];

  if (maisEstourada && maisEstourada.uso > 100) {
    linhas.push(
      `Prioridade: revisar os gastos em "${maisEstourada.label}", que ultrapassou o alocado no período.`,
    );
  } else if (taxaMedia > 85) {
    linhas.push(
      "Reduzir custos variáveis e de conforto para ampliar a margem de saldo mensal.",
    );
  } else {
    linhas.push(
      "Comportamento financeiro estável. Considere aumentar a reserva de emergência ou o aporte em independência financeira.",
    );
  }

  if (menorUso) {
    linhas.push(
      `A categoria "${menorUso.label}" foi pouco utilizada (${menorUso.uso.toFixed(0)}% do alocado) — avalie se a meta está bem calibrada.`,
    );
  }
...
function saveInvClasses(cls) {
  localStorage.setItem("fin_inv_classes", JSON.stringify(cls));
}
...
function getInvLancados(m, y) {
  ...
  const indep = catIndep
    ? catIndep.items.reduce((s, it) => s + (parseFloat(it.value) || 0), 0)
    : 0;
  const emerg = catEmerg
    ? catEmerg.items.reduce((s, it) => s + (parseFloat(it.value) || 0), 0)
    : 0;
...
function renderInvest() {
  ...
  document.getElementById("inv-month-lbl").textContent =
    `${MONTHS[curMonth]} ${curYear}`;
...
function renderInvAporteRows(indep, emerg, total) {
  ...
  function buildRows(cat, catLabel, dotColor) {
    ...
    const subtotal = cat.items.reduce(
      (s, it) => s + (parseFloat(it.value) || 0),
      0,
    );
    return `
...
function renderInvClasses(classes, aporte) {
  ...
  document.getElementById("inv-pct-lbl").textContent =
    `${totalPct.toFixed(0)}% alocados`;
  ...
  classes.forEach((cls, ci) => {
    ...
    div.querySelector(".cat-pct-input").addEventListener("change", (e) => {
      classes[parseInt(e.target.dataset.ci)].pct =
  ...
  addWrap.style.cssText =
    "padding:0.6rem 1.25rem;border-top:0.5px solid var(--border)";
...
function renderInvPie(classes, aporte) {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) =>
                `${fmt(ctx.raw)} (${aporte > 0 ? Math.round((ctx.raw / aporte) * 100) : 0}%)`,
            },
          },
        },
      },
  ...
  document.getElementById("inv-legend").innerHTML = classes
    .map(
      (cls, ci) => `
    <div class="leg-row">
